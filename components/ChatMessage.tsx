...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, onRegenerate }) => {
  ...
  const htmlContent = useMemo(() => {
    ...
    const rawHtml = marked.parse(message.text, { breaks: true, gfm: true, renderer }) as string;
    ...
    const processedHtml = rawHtml.replace(/<a href="(\d+)">([\s\S]*?)<\/a>/g, (match, href, text) => {
        ...
        if (source) {
            ...
            return `
                <span class="group inline items-center gap-1.5 align-middle">
                  <span class="transition-colors group-hover:bg-blue-100 dark:group-hover:bg-blue-900/30 rounded">
                    ${text}
                  </span>
                  <a href="${safeUri}" target="_blank" rel="noopener noreferrer" title="${safeTitle}"
                     class="inline-flex items-center border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-zinc-950 focus:ring-offset-2
                            border-transparent bg-zinc-100 text-black hover:text-black hover:bg-zinc-100/80 dark:bg-zinc-800 dark:text-zinc-50 dark:hover:bg-zinc-800/80 rounded-full no-underline truncate">
                    ${domain}
                  </a>
                </span>
            `;
