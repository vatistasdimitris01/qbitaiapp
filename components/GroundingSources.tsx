...
/**
 * Extracts hostname for favicon retrieval.
 */
...
/**
 * Extracts a cleaner display label for the source domain.
 */
...
/**
 * GroundingSources component: Displays the web and map search sources used by Gemini
 * to ground its response in factual information. Includes a modal view for all sources.
 */
const GroundingSources: React.FC<{ chunks: GroundingChunk[]; t: (key: string) => string; }> = ({ chunks, t }) => {
    ...
    // Show up to 3 icons in the collapsed pill view
            ...
            >
                <div className="flex items-center -space-x-2">
                    {visiblePills.map((chunk, index) => { 
                        ...
                        const iconUrl = isWeb ? `https://www.google.com/s2/favicons?sz=64&domain_url=${getHostname(chunk.web.uri)}` : null; 
                        return (
                            <div key={index} className="size-5 rounded-full bg-white dark:bg-[#141414] border-2 border-white dark:border-[#141414] ring-1 ring-gray-200 dark:ring-[#27272a] overflow-hidden flex items-center justify-center">
                                {iconUrl ? <img src={iconUrl} alt="" className="size-3" /> : <MapPinIcon className="size-3 text-blue-500" />}
                            </div>
                        ); 
                    })}
                </div>
                ...
                <div className="fixed inset-0 bg-black/40 dark:bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4 animate-fade-in-up" onClick={() => setIsModalOpen(false)}>
                    <div className="bg-white dark:bg-[#141414] rounded-[2.5rem] shadow-2xl w-full max-w-md max-h-[75vh] flex flex-col overflow-hidden border border-gray-200 dark:border-[#27272a]" onClick={e => e.stopPropagation()}>
                        <header className="flex items-center justify-between p-7 pb-2">
                            ...
                            <button onClick={() => setIsModalOpen(false)} className="p-2.5 rounded-full bg-gray-50 dark:bg-[#1f1f1f] hover:bg-gray-100 dark:hover:bg-[#292929] transition-colors border border-gray-100 dark:border-[#27272a]">
