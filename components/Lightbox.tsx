  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-[200] flex flex-col items-center justify-center p-4 backdrop-blur-sm animate-fade-in-up" role="dialog" aria-modal="true" onClick={onClose}>
      <header className="absolute top-0 left-0 right-0 flex items-center justify-between p-4 text-white z-10">
        <div className="font-mono text-sm bg-black/20 px-2 py-1 rounded-md">{currentIndex + 1} / {images.length}</div>
        <button onClick={onClose} aria-label="Close" className="p-2 rounded-full bg-black/20 hover:bg-white/20"><XIcon className="size-6" /></button>
      </header>
